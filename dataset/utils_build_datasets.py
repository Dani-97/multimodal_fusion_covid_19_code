...
class Super_Class_Build_Dataset():
    ...
    def __get_only_first_timepoints__(self, features_df):
    ...
    def __build_dataset_scenario_III_aux__(self): 
        ...
        global_merged_features_df = self.__get_only_first_timepoints__(global_merged_features_df)
...
class Build_Dataset_VGG_16_Model_All_Cases(Super_Class_Build_Dataset):

    def __init__(self, **kwargs):
        self.input_csv_file_path = kwargs['input_csv_file_path']
        super().__init__(input_csv_file_path=self.input_csv_file_path)
