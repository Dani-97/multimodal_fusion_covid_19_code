...
class No_Preprocessing():
    ...
    def execute_preprocessing(self, input_data, attrs_headers, params):
...
class Standardization_Preprocessing(No_Preprocessing):
    ...
    def execute_preprocessing(self, input_data, attrs_headers, params):
        ...
        for current_column_aux in range(0, nofcolumns):
            if (params is None):
                current_mean_value = np.mean(input_data[:, current_column_aux])
                current_std_value = np.std(input_data[:, current_column_aux])
            else:
                mean_values, std_values = params['mean'], params['std']
                current_mean_value = mean_values[current_column_aux]
                current_std_value = std_values[current_column_aux]

            if ((current_mean_value + current_std_value)>0):
                preprocessed_data[:, current_column_aux] = \
                    (preprocessed_data[:, current_column_aux] - current_mean_value)/current_std_value
            else:
                preprocessed_data[:, current_column_aux] = np.array([0]*len(preprocessed_data[:, current_column_aux]))
